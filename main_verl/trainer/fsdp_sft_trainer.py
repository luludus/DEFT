...
from main_verl.utils.dataset import SFTDataset
    ...
    def _compute_loss_and_backward(self, batch, do_backward=True):
        ...
        with context, torch.autocast(device_type=self.device_name, dtype=torch.bfloat16):
            if not use_sp:
                ...
                if self.config.trainer.objective_trans == "original":
                    loss = loss_fct(shift_logits, shift_labels)
                    loss = loss * loss_mask.to(loss.device)
                else:
                     
                    log_p_true = F.log_softmax(shift_logits.float(), dim=-1).gather(1, shift_labels.unsqueeze(-1)).squeeze(-1)
                    prob_coefficients = log_p_true.exp()

                    if self.config.trainer.objective_trans == "p": 
                        loss = 1 - prob_coefficients
                        loss = loss * loss_mask.to(loss.device)
                    elif self.config.trainer.objective_trans == "logp":
                        loss = - log_p_true
                        loss = loss * loss_mask.to(loss.device)
                    elif "GeneralFamily" in self.config.trainer.objective_trans:
                        q = float(self.config.trainer.objective_trans.split("-")[1])
                        loss = - (torch.exp(q * log_p_true.float()) - 1) / q 
                        loss = loss * loss_mask.to(loss.device)
                    elif "OnlyTopP" in self.config.trainer.objective_trans:
                        ...
                        loss = 1 - prob_coefficients
                        loss = loss * loss_mask.to(loss.device)
                    elif "OnlyBottomP" in self.config.trainer.objective_trans:
                        ...
                        loss = loss_fct(shift_logits, shift_labels) # Original implementation 
                        loss = loss * loss_mask.to(loss.device)
                    elif "DEFT" in self.config.trainer.objective_trans:
                        params = self.config.trainer.objective_trans.split("-")
                        all_probs = F.softmax(shift_logits.float(), dim=-1)
                        concentration = (all_probs.pow(2)).sum(dim=-1)  
                        alpha = concentration
                        alpha = alpha.detach()
                        loss = -(torch.exp(alpha * log_p_true.float()) - 1) / (alpha + 1e-6)
                        loss = loss * loss_mask.to(loss.device)
                    elif "Cayley_Trans" in self.config.trainer.objective_trans:
                        p_label = prob_coefficients.detach()
                        valid_mask = loss_mask.bool()
                        if valid_mask.sum() > 0:
                            p_mean_global = p_label[valid_mask].mean()
                        else:
                            p_mean_global = torch.tensor(0.0, device=p_label.device)
                        alpha_floor = 0.5 * p_mean_global
                        dist_term = (1.0 - p_label + 1e-8)
                        alpha_local = (1.0 - dist_term) / (1.0 + dist_term + 1e-8)
                        alpha = alpha_floor + (1.0 - alpha_floor) * alpha_local
                        alpha = alpha.clamp(min=0.001, max=1.0)
                        alpha = alpha.detach()
                        loss = -(torch.exp(alpha * log_p_true.float()) - 1) / (alpha + 1e-8)
                        loss = loss * loss_mask.to(loss.device)
                    else:
...
def run_sft(config):
    ...
    train_dataset = create_sft_dataset(config.data.train_files, config, tokenizer)
    val_dataset = create_sft_dataset(config.data.val_files, config, tokenizer)
...
def create_sft_dataset(data_paths, config, tokenizer):
    """Create a dataset."""
    # build dataset
    ...
    dataset = dataset_cls(parquet_files=data_paths, tokenizer=tokenizer, config=config)
    return dataset
