...
LLAMA_MODELS = ['meta-llama/Llama-3.1-8B', 'meta-llama/Llama-3.2-3B']
# LLAMA_INSTRUCT_TOKENISSER = "meta-llama/Llama-3.1-8B-Instruct"
...
class SFTDataset(Dataset):
    ...
    def __init__(self, parquet_files: Union[str, ListConfig], tokenizer, config):
        ...
        # self.use_original_prompt = config.get("use_original_prompt", False)

        if "data" in config:
            self.tokenizer_name = config.model.partial_pretrain
            self.use_llama_base = ("llama" in self.tokenizer_name.lower()) and ("instruct" not in self.tokenizer_name.lower())
            self.use_llama_base_8b = ("llama" in self.tokenizer_name.lower()) and ("8b" in self.tokenizer_name.lower())
            self.use_deepseek_base = ("deepseek" in self.tokenizer_name.lower()) and ("instruct" not in self.tokenizer_name.lower())
            # Now we use the data config 
            config = config.data 
        else:
            self.use_llama_base = False
    ...
    def __getitem__(self, item):
        ...
        if self.use_deepseek_base:
            prompt_chat_str = DEEPSEEK_BASE_BOXED.format(input=prompt)
        elif self.use_llama_base:
            prompt_chat_str = LLAMA_BASE_BOXED_WITH_BOS.format(input=prompt)
        elif self.use_original_prompt:
